...
if [[ -z $HADOOP_HOME ]] ; then
  test -z "$HADOOP_PREFIX"      && export HADOOP_PREFIX=$(find -L /usr/hdp -maxdepth 2 -name hadoop -type d)
else
...
test -z "$JAVA_HOME"             && export JAVA_HOME=/usr/jdk64/jdk1.8.0_60
...
test -z "$ACCUMULO_TSERVER_OPTS" && export ACCUMULO_TSERVER_OPTS="${POLICY} -Xmx4g -Xms1g -XX:NewSize=256m -XX:MaxNewSize=256m "
test -z "$ACCUMULO_MASTER_OPTS"  && export ACCUMULO_MASTER_OPTS="${POLICY} -Xmx2g -Xms256m"
...
test -z "$ACCUMULO_OTHER_OPTS"   && export ACCUMULO_OTHER_OPTS="-Xmx512m -Xms128m"
